#!/usr/bin/env python3
"""Phase 6.4.10 — Integrated Default Verification Benchmark.

Verify the real adopted DoublesDamageAwareConfig() defaults after Phase 6.4.4e
forced switch replacement safety adoption.

Arms:
  A) Default bot vs DoublesBasicAwarePlayer — 300 battles
  B) Default bot vs DoublesSafeRandomPlayer — 100 battles
  C) Default bot vs Default mirror — 300 battles
"""
import asyncio
import csv
import json
...
def _count_audit_metrics(log_path):
    """Count all relevant metrics from JSONL."""
    m = {
...
async def main():
    import argparse
    parser = argparse.ArgumentParser(description="Phase 6.4.10 Integrated Default Verification")
    parser.add_argument("--artifact-tag", default="integrated_default_verification_phase6410",
    ...
    # Use the REAL adopted defaults - no overrides
    config_default = DoublesDamageAwareConfig()

    results = {}
    arms = [
        ("A", "Default vs Basic", config_default, "basic", None, 300),
        ("B", "Default vs SafeRandom", config_default, "safe_random", None, 100),
        ("C", "Default vs Mirror", config_default, "mirror", config_default, 300),
    ]
    ...
    for k in sorted(results.keys()):
        ...
        print(f"  {r['name']:30s} | {r['status']:8s} | {r['finished']}/{r['planned']} | "
              ...
              f"AllImm: {r['all_target_immune_spread_selected']} | "
              f"RepProt: {r['repeat_protect_count']} | "
