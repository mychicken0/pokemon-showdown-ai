#!/usr/bin/env python3
"""Phase WT-4e — Forced Active-Setter Activation Smoke.

A targeted local-only smoke that uses custom teams
with FAST setter Pokemon (Choice Scarf) so the
setter is the lead in turn 1.

Three modes:
* rain: Politoed (Choice Scarf, Rain Dance + Hydro Pump)
  vs opp with no Water/Thunder/Hurricane moves
* sun: Arcanine (Choice Scarf, Sunny Day + Flare Blitz)
  vs opp with no Fire/Solar Beam moves
* terrain: Jolteon (Choice Scarf, Electric Terrain +
  Thunderbolt) vs opp with no Electric moves

The key difference from WT-4d: the setter Pokemon
has Choice Scarf (max speed with positive nature)
so it WILL be the lead in turn 1.
...
# Our teams: fast setter Pokemon with Choice Scarf
OUR_TEAM_FILES = {
    "rain": "data/curated_teams/custom/wt4e_rain_fast_setter_team.json",
    "sun": "data/curated_teams/custom/wt4e_sun_fast_setter_team.json",
    "terrain": "data/curated_teams/custom/wt4e_terrain_fast_setter_team.json",
}

# Opp teams: favorable (from WT-4d)
OPP_TEAM_FILES = {
...
async def run_single_battle(
    ...
    # Check turn 1 active Pokemon
    turn1_active = []
    for tag, r in getattr(bot, "_wt3_decisions", {}).items():
        for d in r:
            if d.get("turn") == 1:
                turn1_active.append(d.get("move_id", ""))
                break
    return {
        "battle_idx": idx,
        ...
        "turn1_active": turn1_active,
    }


def _analyze_audit(
    audit_path: str, mode: str
) -> Dict[str, Any]:
    ...
    stats: Dict[str, Any] = {
        ...
        "n_legal_setters": 0,
        "n_setter_selected": 0,
        "n_status_penalty_exception": 0,
        "selected_examples": [],
        "setter_active_turns_examples": [],
    }
    if not os.path.exists(audit_path):
        return stats
    setter_move = MODE_SETTER_MOVE.get(mode, "")
    with open(audit_path) as f:
        for line in f:
            ...
            for turn in battle.get("audit_turns", []):
                stats["n_turns"] += 1
                # Check if setter is in active Pokemon's
                # moveset (i.e., the setter Pokemon is
                # active)
                for sk in (
                    ...
                    for k in turn.get(sk, []):
                        if (
                            ...
                            and len(k) >= 2
                            and str(k[0]) == "move"
                            and _norm_move_id(k[1]) == setter_move
                        ):
                            stats["n_setter_active_turns"] += 1
                            if len(
                                stats[
                                    "setter_active_turns_examples"
                                ]
...
async def run_mode(
    ...
    for idx in range(1, battles + 1):
        try:
            ...
            print(
                ...
                f"finished={r['bot_finished']} | "
                f"turn1_active={r.get('turn1_active', [])}",
                flush=True,
            )
    ...
    stats = _analyze_audit(audit_path, mode)
    ...
    print(
        ...
        f"setter_active_turns: {stats['n_setter_active_turns']} | "
...
def main():
    ...
    parser.add_argument(
        "--output",
        default="logs/wt4e_active_setter_activation.json",
    )
    parser.add_argument(
        "--audit-dir",
        default="logs/wt4e_active_setter_audits",
    )
    ...
    our_teams = {}
    opp_teams = {}
    for mode in OUR_TEAM_FILES:
        with open(OUR_TEAM_FILES[mode]) as f:
            our_teams[mode] = json_to_showdown(json.load(f))
    for mode in OPP_TEAM_FILES:
        with open(OPP_TEAM_FILES[mode]) as f:
    ...
    print("=" * 60)
    print("WT-4e forced active-setter activation smoke")
    print(
        f"  modes: {modes} | "
    ...
    output = {
        ...
        "summary": {
            ...
            "modes_with_setter_active": sum(
                1
                for r in all_results
                if r["stats"]["n_setter_active_turns"] > 0
            ),
    ...
    print()
    print("=" * 60)
    print("WT-4e forced active-setter smoke complete")
    print("=" * 60)
    print(f"  Output: {args.output}")
    print(
        f"  Modes with setter active: "
        f"{output['summary']['modes_with_setter_active']}/"
