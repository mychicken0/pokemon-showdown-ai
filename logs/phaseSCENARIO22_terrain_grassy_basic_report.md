# Phase SCENARIO-22 — Grassy Terrain

## 1. Summary

SCENARIO-22 implements the grassy variant
of the terrain family:
``terrain_grassy_basic.json``. The scripted
opp leads with Tsareena (Grassy Terrain
user) + Arcanine (Protect) and uses
Grassy Terrain on turn 1.

**Decision**: ``GRASSY_BASIC_PASS``.

Both battles have Grassy Terrain and Protect
...
|---|---|---|
| 97280 | terrain_grassy_basic | (1, 0, grassyterrain), (1, 1, protect) |
| 97281 | terrain_grassy_basic | (1, 0, grassyterrain), (1, 1, protect) |

Both battles have Grassy Terrain and
Protect executed.
...
```
grassyterrain_actually_used: canonical=True gap=True
no_script_failures: passed
```

## 4. Scenario file

``data/curated_teams/scenarios/terrain_grassy_basic.json``:

- **scenario_id**: ``terrain_grassy_basic``
- **our_team_file**:
  ``data/curated_teams/control4a/team_020.json``
- **opp_team_file**:
  ``data/curated_teams/custom/grassy_demo_v1.json``
- **lead**: opp_slot_0=Tsareena,
  opp_slot_1=Arcanine
- **script**: turn_1: opp_slot_0=grassyterrain,
  opp_slot_1=protect
- **validators**:
  - ``expected_scripted_action
    { field: grassyterrain, expected: true }``
  - ``no_script_failures``
...
1. **Tsareena** (default ability, Leftovers)
   — the Grassy Terrain user
...
Grassy Terrain user. A custom team was
created with the Tsareena mon (which
learns Grassy Terrain via TM in Gen 9).
...
## 9. Library status (post-SCENARIO-22)

- ✅ terrain_grassy_basic (SCENARIO-22, Grassy Terrain)
...
| source | path | role |
|---|---|---|
| Scenario | ``data/curated_teams/scenarios/terrain_grassy_basic.json`` | NEW v1 → v2 |
| Custom team | ``data/curated_teams/custom/grassy_demo_v1.json`` | NEW |
| Opp team | ``data/curated_teams/custom/grassy_demo_v1.json`` | Tsareena |
