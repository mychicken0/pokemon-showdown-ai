...
# Define the variants to test (300 battles each)
VARIANTS = {
    "full_phase2": DoublesDamageAwareConfig(),
    "no_spread_intelligence": DoublesDamageAwareConfig(enable_spread_intelligence=False),
    "no_focus_fire_bonus": DoublesDamageAwareConfig(enable_focus_fire_synergy=False),
    "no_spread_no_focus_fire": DoublesDamageAwareConfig(enable_spread_intelligence=False, enable_focus_fire_synergy=False),
    "no_spread_old_ko_250": DoublesDamageAwareConfig(enable_spread_intelligence=False, ko_bonus=250.0),
    "no_focus_fire_old_ko_250": DoublesDamageAwareConfig(enable_focus_fire_synergy=False, ko_bonus=250.0),
    "no_spread_low_focus_fire_30": DoublesDamageAwareConfig(enable_spread_intelligence=False, focus_fire_synergy_bonus=30.0),
    "conservative_spread_penalty": DoublesDamageAwareConfig(
        enable_spread_intelligence=True,
        ally_hit_penalty=150.0,
        spread_bonus=30.0
    )
}

async def run_variant(name, config, n_battles=300):
    rand_suffix = random.randint(1000, 9999)
    player = DoublesDamageAwarePlayer(
        account_configuration=AccountConfiguration(f"Champ_{rand_suffix}", None),
        verbose=False,
        config=config,
    ...
    print(f"\nRunning champion variant '{name}' against DoublesBasicAwarePlayer for {n_battles} battles...")
    ...
    print(f"[{name}] Wins/Losses: {wins}/{losses} | Win Rate: {win_rate:.2f}% | Avg Turns: {avg_turns:.2f} | Protect: {avg_protect:.2f} | FakeOut: {avg_fake_out:.2f} | Spread: {avg_spread:.2f} | FocusFire: {avg_focus_fire:.2f}")
...
async def main():
    print("Starting Doubles Phase 2 Champion Config Test Suite...")
    
    results = []
    for name, config in VARIANTS.items():
        res = await run_variant(name, config, n_battles=300)
        results.append(res)
    ...
    print(f"\nChampion config results saved to: {csv_file}")
