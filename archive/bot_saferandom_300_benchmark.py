#!/usr/bin/env python3
"""Phase 6.4.10a — SafeRandom Confirmation Benchmark.

Default bot vs DoublesSafeRandomPlayer — 300 battles
"""
import asyncio
import csv
import json
...
async def run_matchup_with_watchdog(name, config, opp_class, opp_config, n_battles, log_path,
                                     arm_timeout=10800, benchmark_arm=""):
    if opp_class == "safe_random":
...
async def main():
    artifact_tag = "integrated_default_followup_phase6410a_saferandom"
    n_battles = 300
    ...
    result = await run_matchup_with_watchdog(
        "Default vs SafeRandom (300)", config_default, "safe_random", None, n_battles,
        f"logs/{artifact_tag}.jsonl",
        benchmark_arm="A",
    )
    ...
    print(f"  {r['name']:35s} | {r['status']:8s} | {r['finished']}/{r['planned']} | "
    ...
    # Quick validation
    with open(f"logs/{artifact_tag}.jsonl") as f:
        records = [json.loads(l) for l in f if l.strip()]
    tags = set(r.get("battle_tag") for r in records)
    outcomes = sum(1 for r in records if r.get("won") is not None)
    print(f"\nValidation: {len(records)} records, {len(tags)} unique tags, {outcomes} outcomes")
    if len(records) == n_battles and len(tags) == n_battles and outcomes == n_battles:
        print("  PASS")
    else:
        print("  FAIL")
